...
from src.config import THINGIVERSE_API_AUTH, THINGIVERSE_API_TOKEN, THINGIVERSE_API_DONE, \
...
def download_models():
    try:
        ...
        thing.download_models(flask.session.get('credentials', {}).get('access_token'))
...
def oauth2_callback():
    try:
        ...
        response = requests.post(THINGIVERSE_API_TOKEN, params=auth_query_parameters)
